...
export default function VideoEditor() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (let entry of entries) {
        ...
        } else if (entry.target === exportRef.current) {
          setExportHeight(entry.target.getBoundingClientRect().height);
        }
      }
    });
    if (resizeRef.current) observer.observe(resizeRef.current);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const rightColumnHeight = resizeHeight + exportHeight + 40;
      const maxOffset = Math.max(0, parentHeight - rightColumnHeight);
      const currentOffset = -parentTop + 60;
      if (parentTop < 60) {
        setStickyOffset(Math.min(maxOffset, currentOffset));
      } else {
        setStickyOffset(0);
      }
    };
  ...
  }, [file, resizeHeight, exportHeight]);
    ...
    <div className="min-h-screen relative flex flex-col" style={{ background: "var(--bg)" }}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 pb-6 flex-1 w-full">
        ...
        <div
          ...
          style={{ minHeight: resizeHeight > 0 || exportHeight > 0 ? `${resizeHeight + exportHeight + 40}px` : undefined }}
        >
          {/*
            LAYOUT: Floated & Sticky columns to maintain the identical two-column UI layout
            on desktop screens while preserving the exact logical DOM/tab order:
            Upload -> Preset -> Framing -> Trim -> Rotate -> Audio -> Quality -> Export
          */}

          {/* BLOCK 1: Upload & Video Preview (Left Column) */}
          <div className="order-1 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5 bg-[var(--surface)] rounded-xl p-3 border border-[var(--border)] animate-fade-in">
          ...
          {file && file.size > 100 * 1024 * 1024 && (
            <div className="order-1 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5">
          ...
          <div
            ref={resizeRef}
            className="order-4 lg:absolute lg:right-0 lg:w-[340px] mb-5 animate-fade-in"
            style={{
              top: resizeHeight > 0 || exportHeight > 0 ? `${stickyOffset}px` : undefined,
              animationDelay: "50ms"
            }}
          >
            {!file && (
            ...
            <div className={cn(
              "bg-[var(--surface)] rounded-xl border border-[var(--border)] p-5 space-y-6",
          ...
          {file && (
            <div className={cn(
              "order-2 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5",
          ...
          {file && (
            <div className={cn(
              "order-3 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5",
              "grid grid-cols-1 gap-4",
              isProcessing && "pointer-events-none opacity-50"
            )}>
              <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] p-5 space-y-6">
          ...
          {status === "error" && error && (
            <div
              role="status"
              className="order-3 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5 flex items-start gap-3 p-4 bg-film-50 border border-film-200 rounded-xl text-film-800 text-sm animate-fade-in"
            >
          ...
          {status === "done" && result && (
            <div role="status" className="order-3 lg:float-left lg:w-[calc(100%-360px)] lg:clear-left lg:mr-5 mb-5 animate-fade-in" ref={downloadRef}>
          ...
          <div
            ref={exportRef}
            className={cn(
              "order-5 lg:absolute lg:right-0 lg:w-[340px] mb-5 transition-opacity duration-300",
              "space-y-5",
              (isProcessing || !file) && "pointer-events-none opacity-50",
              resizeHeight === 0 && "lg:opacity-0 lg:pointer-events-none"
            )}
            style={{
              top: resizeHeight > 0 ? `${stickyOffset + resizeHeight + 20}px` : undefined
            }}
          >
