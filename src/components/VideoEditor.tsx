...
export default function VideoEditor() {
  const {
    ...
    result, batchResults, error, exportStartedAt, updateRecipe,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-5">
          ...
          <div className={cn(
            ...
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] p-5 space-y-6 animate-fade-in" style={{ animationDelay: "50ms" }}>
              {/* Recommended preset banner — shown in single-export mode only */}
              {recommendedPreset && !batchMode && (
                <div className="mb-4 rounded-2xl border border-film-200 bg-film-50 p-3 text-sm text-film-700">
                  <p>
                    ...
                    {(recommendedPreset.platform.split("·")[0] ?? "").trim()} ({recommendedPreset.label.replace(/\s/g, "")})
                  </p>
                </div>
              )}

              {/* Batch export panel handles preset selection in both single and batch modes */}
              <BatchExportPanel
                recipe={recipe}
            ...
            >
              <Zap size={20} className={cn(!exportBlocked && "animate-pulse")} />
