...
const FORMAT_OPTIONS = [
  ...
  { id: "mkv", label: "MKV", description: "Container, maximum quality" },
  {
    id: "gif",
    label: "GIF",
    description: "Animated image — keep clips under 10 s",
  },
  { id: "mp3", label: "MP3", description: "Extract audio only" },
] as const;
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
        {FORMAT_OPTIONS.map((option) => (
          <button
            ...
            onClick={() =>
              onChange({
                format: option.id as "mp4" | "webm" | "mkv" | "gif" | "mp3",
              })
            }
            aria-label={`Select ${option.label} format`}
            aria-pressed={recipe.format === option.id}
            className={cn(
              "relative px-2 py-2.5 rounded-lg border-2 transition-all",
