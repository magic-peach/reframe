...
function buildArguments(
  ...
  const vf = isMp3 ? "" : buildVideoFilter(recipe, targetW, targetH);
  ...
  if (recipe.fps > 0 && !isMp3) {
    args.push("-r", String(recipe.fps));
  }
...
async function runExport(request: ExportRequest): Promise<ResultPayload> {
  ...
  try {
    ...
    if (exitCode !== 0) {
      const isMp3Fallback = recipe.format === "mp3";
      const fallbackFormat = isMp3Fallback ? "mp3" : "webm";
      const currentFallbackName = isMp3Fallback
        ? `fallback_${sessionId}.mp3`
        : fallbackOutputName;
      ...
      const data = await ffmpeg.readFile(currentFallbackName, undefined, {
        signal: activeExportAbortController?.signal,
      });
      const payload = (data as Uint8Array).buffer as ArrayBuffer;
      return {
        ...
        id: sessionId,
        data: payload,
        mimeType: isMp3Fallback ? "audio/mpeg" : "video/webm",
        size: payload.byteLength,
        width: targetW,
        height: targetH,
