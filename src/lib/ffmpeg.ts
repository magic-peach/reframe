...
function buildArguments(
  ...
  if (needsFilterComplex) {
    ...
    if (hasOverlay) {
      ...
      // Convert to unknown first to safely bypass strict type overlap checks
      const coords = overlayOptions!.position as unknown as {
        x: number;
        y: number;
      };
      const overlayX = `(main_w)*${coords.x}/100`;
      const overlayY = `(main_h)*${coords.y}/100`;

      filterParts.push(
        `[${overlayIdx}:v]scale=${scaledW}:-2,format=rgba,colorchannelmixer=aa=${alpha}[logo]`,
      );
      filterParts.push(
        `${videoOut}[logo]overlay=${overlayX}:${overlayY}[vout]`,
      );
      videoOut = "[vout]";
    }
