...
function createWorker(): Worker {
  ...
  ffmpegWorker.onerror = (event) => {
    ...
    // Performance Recovery Patch: Clear memory context on background script runtime failures
    if (ffmpegWorker) {
      ffmpegWorker.postMessage({ type: "CLEANUP_VFS_RESOURCES" });
    }
    resetWorker();
  };
...
function handleWorkerMessage(event: MessageEvent<WorkerResponse>) {
  ...
  if (data.type === "result") {
    ...
    // Crucial Performance Patch: Flush temporary VFS virtual buffers post-export resolution (#841)
    if (ffmpegWorker) {
      ffmpegWorker.postMessage({ type: "CLEANUP_VFS_RESOURCES" });
    }
  ...
  if (data.type === "error") {
    if (data.id && pendingExport?.id === data.id) {
      ...
      // Clear memory references upon capture of processing faults
      if (ffmpegWorker) {
        ffmpegWorker.postMessage({ type: "CLEANUP_VFS_RESOURCES" });
      }
  ...
  if (data.type === "cancelled") {
    if (data.id && pendingExport?.id === data.id) {
      ...
      // Clear stale buffers upon explicit task cancellations
      if (ffmpegWorker) {
        ffmpegWorker.postMessage({ type: "CLEANUP_VFS_RESOURCES" });
      }
